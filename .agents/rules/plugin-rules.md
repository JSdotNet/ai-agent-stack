...
where the sync reads them and where every rule's scope can be seen at once.

```json
{ "rules": { "devbook-arc42": { "paths": [".arc42/**", ".devbook/arc42/**"], "sync": "arc42" } } }
```

`sync` is optional and belongs to the plugin that materializes rules; devbook reads it as the
...
| a path in the consuming repository | `.devbook/domain/**` | the plugin's sync installs it there, per [the decision](../../.devbook/arc42/09-architecture-decisions.md#a-plugins-rules-reach-a-host-through-the-sync) |
...
it resolves both here and wherever the sync writes them, because the folder shape is the same
in both places. That is what the `rules/<name>.md` naming buys.

A repo-facing rule with no sync behind it reaches a session by explicit path and nothing else.
