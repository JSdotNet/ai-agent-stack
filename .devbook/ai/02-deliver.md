...
- **Used for** — every category of change to a repository that has the engine enabled:
  ...
  `delivery-canvas`, or `delivery-collector` — and resolves its host slots from the
  `claude-desktop` profile: `CLAUDE.md` as `repo-instructions`, sub-agents as
  `stage-delegation`, `delivery-dashboard` as `surface`.
