...
```meta
type: dependencies
related: [".devbook/domain/context-map.md#delivery-schedule", ".devbook/arc42/09-architecture-decisions.md#the-unattended-lane-is-its-own-plugin"]
```
...
|---|---|---|---|---|
| [Delivery](../delivery/dependencies.md) | Customer-Supplier, declared `delivery >=2.0.0 <3.0.0` | Its entry points call the engine's flows and phases | `resources/flow-phases.md`, `resources/surface-contract.md`, the parking rule at a gate | The entry points are adapters onto flows. The dependency is real, and it is the only declared one. |
...
- **The host-capability divergence is recorded, not hidden.** Nothing else in this marketplace
  ...
  [the decision](../../arc42/09-architecture-decisions.md#the-unattended-lane-is-its-own-plugin).
