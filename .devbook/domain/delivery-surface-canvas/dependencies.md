...
| Plugin Authoring | Shared Kernel, with one documented exception | Plugin folder and one manifest; **no marketplace entry** | [naming.md](../plugin-authoring/naming.md) | A plugin ships the manifest of every host that can load something in it. Here that is one host, so the folder-shape rule is satisfied rather than broken. |
