...
```meta
type: dependencies
related: [".devbook/domain/context-map.md#devbook", ".devbook/arc42/09-architecture-decisions.md#flows-belong-to-delivery", ".devbook/arc42/tdr/4-delivery-depends-on-devbook.md"]
```
...
|---|---|---|---|---|
| [Devbook Collaboration](../devbook-collaboration/dependencies.md) | Customer-Supplier, declared | `ext.devbook-collaboration.*` keys in a chapter's own block; writes devbook's `approved` rung | The `ext` namespace and the `status` ladder | That `ext.*` is carried through untouched and unvalidated, and that `approved`, `approved-by`, and `approved-at` keep their meaning. |
