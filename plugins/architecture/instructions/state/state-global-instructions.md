...
- [ ] States are named using domain language.
- [ ] Every transition is labelled with an event; guards and actions added where relevant.
- [ ] Initial (`[*]`) and terminal states are explicit.
- [ ] Composite states used for nested lifecycles rather than duplicating transitions.
- [ ] Diagram is embedded in a fenced `mermaid` code block.
- [ ] Prose summary accompanies the diagram.
- [ ] Traceability links to arc42 §6 or §8 and related domain artifacts are present.
