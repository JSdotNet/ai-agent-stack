...
`schedule-sync` in the repository.
...
| Skill | Does |
|---|---|
| `schedule-sync` | Creates or updates the selected schedules, disables the deselected, writes `components.schedule` |
...
the stamp, and the operations are in `instructions/schedule-catalog-contract.instructions.md`.
...
it and the plugins a schedule requires. `schedule-sync` checks that and refuses to schedule
