...
3. Never push to `{{base}}`. Land every change as a pull request from the branch `schedule/{{name}}/<YYYY-MM-DD>`: ready for review when build and tests passed, draft otherwise. Never merge, never approve, never close, never delete anything.
