---
name: schedule-run
description: 'Fire one of this repository''s schedules now, outside its cadence, and report the run. Use when: running a routine or automation now, testing one right after schedule-sync, or re-running one that failed.'
---

# schedule run

The scheduler and the identity rule are in `instructions/schedule-catalog-contract.instructions.md`.
