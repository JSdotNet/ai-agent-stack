// Shared state resolution for the delivery-surface-dashboard MCP server and its telemetry hook.
//
...
export function stateDir() {
    if (process.env.DELIVERY_SURFACE_DASHBOARD_STATE_DIR) {
        return path.resolve(process.env.DELIVERY_SURFACE_DASHBOARD_STATE_DIR);
    }
    ...
    return path.join(home, "delivery-surface-dashboard", `${slug}-${hash}`);
}
