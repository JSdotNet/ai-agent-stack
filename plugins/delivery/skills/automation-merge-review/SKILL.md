...
- Never call `gh pr review --approve` or `--request-changes`, never `gh pr merge`, and never
  push to any branch. This automation writes exactly one thing: a comment.
...
- One comment per head commit. A pull request whose head already carries this automation's
...
2. Drop drafts unless included, apply the base branch filter, and drop every pull request whose
   comments already contain `<!-- automation-merge-review: <headRefOid> -->`:
...
8. When posting is on, leave one comment per reviewed pull request, marker first:
   ...
   ```markdown
   <!-- automation-merge-review: <headRefOid> -->
   ...
   *Posted by `automation-merge-review` on <ISO datetime UTC>. A person approves and merges.*
   ```
...
- `start_run` with `skillId: "automation-merge-review"` and these stages: Collect, Review
...
- `pr-merge-ready` is the attended counterpart: it fixes what this automation only reports.
  Pair them by hand, never on one schedule, or the fixer races the reviewer.
- A pull request opened by another automation is reviewed like any other. Nothing here reads
  who opened it.
