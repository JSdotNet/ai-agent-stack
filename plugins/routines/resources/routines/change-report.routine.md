...
cadence: weekly
cron: "0 15 * * 5"
target: delivery:automation-whats-new
requires: [delivery]
tools: [Bash, Read, Write, Glob, Grep, Skill]
---

Run `automation-whats-new` for the repository `{{repo}}` with the base branch filter
...
Publish the report as the routine-report issue. Skip the follow-up phase — there is nobody to
