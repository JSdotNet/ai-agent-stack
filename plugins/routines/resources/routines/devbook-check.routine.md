...
Run `devbook-check` from the repository root over every adopted knowledge folder.
...
migration ledger or the stamp reports hard drift, do not repair it — `devbook-sync` owns that
and it needs a person. Publish a routine-report issue naming the outstanding migrations or the
drift, unless one is already open.

One routine covers every folder. The generator walks them in one pass and a stale reference
