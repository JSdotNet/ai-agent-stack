#!/usr/bin/env node
// check.mjs — validate the routine catalog against routine-catalog-contract.instructions.md.
//
...
const CATALOG = path.join(PLUGIN, "resources", "routines");
const PREAMBLE = path.join(PLUGIN, "resources", "routine-preamble.md");
...
const files = (await readdir(CATALOG)).filter((f) => f.endsWith(".routine.md")).sort();
...
for (const name of files) {
    ...
    const stem = name.replace(/\.routine\.md$/, "");
    ...
    if (typeof fields.target === "string") {
        ...
        if (!m) error(file, `target ${fields.target} is not <plugin>:<skill>`);
            ...
            targetPlugin = m[1];
            if (m[2].startsWith("flow-")) error(file, `target ${fields.target} is a flow — a flow ends at a gate no routine can pass`);
...
console.log(`routine catalog: ${files.length} routine(s), ${errors.length} error(s).`);
